...
public class LogisticsController {
    ...
    public ReturnInfor getInfo(@RequestParam("ShipperCode") String shipperCode, @RequestParam("LogisticCode") String logisticCode){
        if (Empty.isNotEmpty(shipperCode) && Empty.isNotEmpty(logisticCode)){
            try {
    ...
    public ReturnInfor placingOrder(@RequestBody List<PlacingOrderInfo> placingOrderInfos){
        ...
        Sender sender = null;
        if (placingOrderInfos.get(0).getReceiver().getAreaCode() != null && placingOrderInfos.get(0).getReceiver().getAreaCode().equals("sz")){
            sender  = senderService.byIsDefault(1);//深圳发货地址
        }else {
            sender = senderService.byIsDefault(0);//江西发货地址
        }
    ...
    public ReturnInfor userQuery(@RequestParam("LogisticCode") String logisticCode,@RequestParam("ShipperCode") String shipperCode){
        if (Empty.isNotEmpty(logisticCode)){
            LogisticsInfo logisticsInfo = logisticsInfoService.userQuery(logisticCode,shipperCode);
